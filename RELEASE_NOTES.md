...
- macOS 13 or newer: universal Apple Silicon and Intel DMG, signed with Developer ID and notarized by Apple.
- Windows 10 or 11 x64: signed MSI and current-user NSIS installers with trusted RFC 3161 timestamps.

The release workflow publishes this note only after both platform jobs verify signatures, notarization or timestamps, and installer behavior. `SHA256SUMS` accompanies the three installers.
...
Before promoting this candidate to 1.0, maintainers must complete the target-device checks in [the release candidate runbook](docs/release-candidate.md), including clean install, upgrade, uninstall, cold start, stable memory, and online dependency review.
