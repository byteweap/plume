...
本文记录 Plume 当前的架构边界，描述的是已经实现的行为，而不是完整目标产品。计划内容请参阅[产品需求文档](产品需求文档.md)和[开发任务分解](开发任务分解.md)。
...
1. React 校验连接表单，并向 Rust 发送短生命周期请求。
2. `test_connection` 打开客户端、验证服务器、返回元数据，然后关闭连接。
3. `connect_database` 打开初始客户端并注册服务器会话。
4. React 只收到不透明会话 ID，密码不会返回 React。
5. 展开其他数据库时，注册表提供对应客户端；已有客户端会复用，否则使用服务器会话配置新建。
6. 会话和连接配置目前只存在于内存中，退出 Plume 后消失。

当前注册表刻意保持简单。连接池、显式断开、重连、查询所有权和取消能力应继续扩展此边界，而不是在命令模块中增加全局可变状态。
